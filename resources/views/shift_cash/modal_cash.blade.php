<div class="px-6 py-2 space-y-5" x-data="{
    ...
    <div class="p-4 rounded-2xl bg-gradient-to-r from-gray-900/5 via-gray-800/5 to-transparent border border-gray-300/60 dark:border-gray-700/60 flex items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-xl bg-[#111827] text-white flex items-center justify-center shadow-sm">
    ...
    <div class="grid gap-6 md:grid-cols-3">
        ...
        <div class="space-y-3">
            <div class="flex items-center gap-2 pb-1 border-b border-gray-200 dark:border-gray-700/60">
                <i class="ri-bar-chart-2-fill text-gray-700 dark:text-gray-300 text-base"></i>
            ...
            <div class="space-y-2">
                @php
                    $group1 = [
                        ...
                        ['id' => 'tips_summary', 'label' => 'Propinas recaudadas por mozo', 'icon' => 'ri-heart-3-line'],
                ...
                @foreach($group1 as $opt)
                    <label for="{{ $opt['id'] }}" class="relative flex items-center justify-between p-3 rounded-xl border border-gray-200 dark:border-gray-700/80 bg-white dark:bg-gray-800/80 transition-all duration-200 hover:border-gray-500 dark:hover:border-gray-500 hover:shadow-sm select-none cursor-pointer group has-[:checked]:border-gray-800 has-[:checked]:bg-gray-100/70 dark:has-[:checked]:bg-gray-800/60 dark:has-[:checked]:border-gray-400">
                        <div class="flex items-center gap-2.5 min-w-0 pr-2">
                            <div class="w-7 h-7 rounded-lg flex items-center justify-center text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700/60 group-hover:text-gray-800 dark:group-hover:text-gray-200 group-has-[:checked]:bg-gray-800 group-has-[:checked]:text-white dark:group-has-[:checked]:bg-gray-200 dark:group-has-[:checked]:text-gray-900 transition-colors shrink-0">
                                <i class="{{ $opt['icon'] }} text-sm"></i>
                            </div>
                            <span class="text-xs font-medium text-gray-700 dark:text-gray-200 group-has-[:checked]:text-gray-950 dark:group-has-[:checked]:text-white leading-tight">
                                {{ $opt['label'] }}
                            </span>
                        </div>
                        <input type="checkbox" name="options[{{ $opt['id'] }}]" value="1" id="{{ $opt['id'] }}" checked @change="updateCount()" class="w-4 h-4 rounded text-gray-800 focus:ring-gray-500 border-gray-300 dark:border-gray-600 dark:bg-gray-700 accent-[#111827] cursor-pointer shrink-0">
                    </label>
                @endforeach
            </div>
        </div>

        {{-- Section 2: Flujos y Métodos --}}
        <div class="space-y-3">
            <div class="flex items-center gap-2 pb-1 border-b border-gray-200 dark:border-gray-700/60">
                <i class="ri-wallet-3-fill text-gray-700 dark:text-gray-300 text-base"></i>
        ...
        {{-- Section 3: Anulaciones y Ajustes --}}
        <div class="space-y-3">
            <div class="flex items-center gap-2 pb-1 border-b border-gray-200 dark:border-gray-700/60">
                <i class="ri-shield-flash-fill text-gray-700 dark:text-gray-300 text-base"></i>
